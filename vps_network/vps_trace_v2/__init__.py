"""
VPS trace V2 impl for `icmplib` async support
"""
...
def init_traceroute_cli_v2(main: click.Group):
    @main.command()
    ...
    def traceroute(host: str, count: int, hops: int, fast: bool):
        ...
        table = Table(title=f"Traceroute ({host}) 测试", show_lines=True)
        ...
        with Live(table, console=console, refresh_per_second=8, transient=True, vertical_overflow="visible"):
            result = do_traceroute(
                table=table, address=host, count=count, max_hops=hops, fast=fast
            )
            print(result)
        print("\n" * 60)
        console.clear()

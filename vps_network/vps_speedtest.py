...
class SpeedTestResult(BaseModel):
    ...
    bytes_send: Optional[int] = Field(None, title="发送字节")
...
def do_speed_test(
    ...
    if disable != "up":
        with Progress(
            ...
            task_id = progress.add_task("上传测试:")

            def up_cb(idx, total, **kwargs):
                speed_test_cb(progress, task_id, idx, total, **kwargs)

            st.upload(threads=up_threads, callback=up_cb, pre_allocate=False)
    if disable != "dl":
        with Progress(
            "[progress.description]{task.description}",
            BarColumn(),
            ...
            task_id = progress.add_task("下载测试:")
